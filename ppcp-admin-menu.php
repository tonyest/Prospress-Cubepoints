			...
			<p class="ppcp-admin-menu general-settings">
				<table class="ppcp-admin-menu form-table">
					<tr valign="top">
						...
						<td valign="middle">
							<input type="text" <?php disabled($ppcp_mode);?> id="win_points" name="win_points[value]" value="<?php echo $win_points['value']; ?>" size="10" ></input>
						</td>					
						<td>
							<input type="checkbox" <?php disabled($ppcp_mode);?> name="win_points[enabled]" <?php checked($win_points['enabled'],'on'); ?> ><?php _e('Add points for winning auctions','cp'); ?>
							</input>
						</td>
					</tr>
					<tr valign="top">
						...
						<td valign="middle">
							<input type="text" <?php disabled($ppcp_mode);?> id="sell_points" name="sell_points[value]" value="<?php echo $win_points['value']; ?>" size="10" /></td>
						<td>
							<input type="checkbox" <?php disabled($ppcp_mode);?> name="sell_points[enabled]" <?php checked($sell_points['enabled'] , 'on' ); ?> ><?php _e('Add points for selling items','cp'); ?></input></td>
					</tr>
					<tr valign="top">
						<th scope="row"><label for="bid_points"><?php _e('Number of points for each winning bid','cp'); ?>:</label></th>
						<td valign="middle">
							<input type="text" <?php disabled($ppcp_mode);?> id="bid_points" name="bid_points[value]" value="<?php echo $bid_points['value']; ?>" size="10" />
						</td>
						<td>
							<input type="checkbox" <?php disabled($ppcp_mode);?> name="bid_points[enabled]" <?php checked($bid_points['enabled'], 'on'); ?> ><?php _e('Add points winning bids','cp'); ?></input>
						</td>
					</tr>
				</table>
			</p>
